%\VignetteIndexEntry{R packages: archivist}
...
\hlstd{iris} \hlopt{%a%}
    \hlstd{dplyr}\hlopt{::}\hlkwd{filter}\hlstd{(Sepal.Length} \hlopt{<} \hlnum{6}\hlstd{)} \hlopt{%a%}
    \hlkwd{lm}\hlstd{(Petal.Length}\hlopt{~}\hlstd{Species,} \hlkwc{data}\hlstd{=.)} \hlopt{%a%}
